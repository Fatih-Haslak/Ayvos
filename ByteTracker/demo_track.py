...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if ret_val:
            img,img_info = predictor.inference(frame,timer)
            timer.tic()
            with torch.no_grad():
                my_outputs= detector.run(frame)
            #print("fato cıktılar",my_outputs)
           
            if my_outputs[0] is not None:
                #print("NE bu",outputs[0].cpu().detach().numpy())
                output_results = my_outputs.cpu().numpy()
                #print(output_results[:,1])
                online_targets = tracker.update(output_results, [img_info["height"], img_info["width"]], exp.test_size)
...
def main(exp, args):
    ...
    predictor = Predictor(exp, args.device)
